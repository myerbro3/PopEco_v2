...
datadir <- paste( getwd(), "/Week_5/Data/", sep = "" )
...
sum(greatdf[ ,c("p1", "p2", "p3") ], na.rm = TRUE)

#Answer: 
# Only 16 total detections in two years, elusive species
...
## Answer: No, all much lower than 0.4, very weak correlation 
###
...
# Answer: forest skewed heavily right
#
...
# Answer: U-shaped histogram for clouds, mostly 0% of 100% 
# 
...
zst <- rep(1, I ) #starting occupancy 
#number of occupancy predictors
B <- 2
#number of detection predictors
A <- 1
#create initial values to start the algorithm # draw random values from normal and alpha
...
m_great <-  autojags( great.data, inits = inits, params, modelname, #
                 ...
                 Rhat.limit = 1.02, #measure of how well each chain is mixing, want around 1
                 save.all.iter = FALSE, parallel = TRUE ) 

###### end m1 ########
##################################################################
### save workspace ###
save.image( "BM_OccBayesResults.RData" )
