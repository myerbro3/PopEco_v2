...
load( "~/R/PopEco_v2/Week_12/MarkedResultsJAGs2.RData" )
################################################################################
...
# alphas = effort, temp, wind  
# beta = habitat predictor
############## whisker plots #############
...
for(i in 1:I){
  ...
  values <-  as.vector(quantile(
    mr$sims.list$N[,i ],probs = c(0.025, 0.975) ) )
  N_df[i,'N.lowCI'] <- values[1]
  N_df[i,'N.highCI'] <- values[2]
}
...
# ANSWER: 
# BOTH MODELS SUCCESSFULLY CONVERGED, HOWEVER THE BAYESIAN P VALUES FOR THE M1 
# MODEL (0.1058) WERE BETTER THAN THE P VALUES FOR THE M2 MODEL (0.0098, 0.0002)

# M2 ESTIMATED LOWER ABUNDANCE (BEGINNING AT 250) THAN M1 (500). 
# BTH MODELS IDENTIFIED A STRONG POSITIVE RELATIONSHIP BETWEEN DETECTION 
# PROBABIITY AND EFFORT. AND A STRONG NEGATIVE RELATIONSHIP BETWEEN DETECTION 
# PROBABILITY AND # MEAN TEMPERATURE. A MINOR NEGATIVE CORRELATION BETWEEN
# DETECTION PROBABILITY AND WIND WAS ALSO IDENTIFIED. 
