...
load("~/R/PopEco_v2/Week_11/MarkPrepWorkspace.RData")
###########################################################################
####################### define MCMC settings ##############################

ni <- 20000; nt <- 20; nb <- 50000; nc <- 5 #iterations, thinning, burnin, chains
...
# detection predictors: int + wind + temp + effort 
############################################################################
...
m1 <- autojags( win.data, inits = inits, params, modelname, #
                  ...
                  iter.increment = ni, max.iter = 500000, # 500,000 MYSELF 
                  Rhat.limit = 1.05, #CHANGE TO 1.05 IF I RUN MYSELF
...
m2 <- autojags( win.data, inits = inits, params, modelname, #
                  n.chains = nc, n.thin = nt, n.burnin = nb,
                  iter.increment = ni, max.iter = 500000, 
                  Rhat.limit = 1.05,
                  save.all.iter = FALSE, parallel = TRUE ) 
...
##### save relevant stuff ##################################
save.image( "~/R/PopEco_v2/Week_12/MarkedResultsJAGs2.RData")
