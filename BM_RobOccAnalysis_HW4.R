...
datadir <- paste( getwd(), "/Data_HW1/", sep = "" )
...
#answer: YES, NUMBER OF YEARS X NUMBER OF SURVEYS  
...
# Answer: BECAUSE THE PSI SUBMODEL REPRESENTS THE INITIAL STATE AND REPRESENTS 
# OCCURENCE AT A SITE DURING SEASON 1. IT IS GOVERNED ONLY BY THE OCCUPANCY 
# PROBABILITY 
#
# View model results:
fm.dyn
...
confint( fm.dyn, type = "psi" )
plogis(confint( fm.dyn, type = "psi" ))

# what does that plogis function do? What does the result tell us?
# Answer: CONFINT CALCULATE THE UPPER AND LOWER CONFIDENCE INTERVALS FOR A 95% 
# CONFIDENCE INTERVAL. CALLING PLOGIS ON THE CONFIDENCE INTERVALS TRANSFORMS THE
# UPPER AND LOWER BOUNDS OF THE CONFIDENCE INTERVALS INTO PROBABILITIES 
#
# Why don't we do the same for colonization and extinction?
# Answer: USING PLOGIS ON THE COL AND EXT COMPONENTS TURNS THE INTERCEPT AND 
# COEFFICIENT ESTIMATES INTO PROBAILITY SCALE. COEFFICIENT ESTIMATES ON A 
# PROBABILITY SCALE ARE MEANINGLESS SO WE DO NOT LOOK AT THOSE HERE. 
#
...
# Answer:
# COL: THE APRMAY.MAXT CI CONTAINS 0, SO THIS PREDICTOR HAS NO RELATIONSHIP
# WITH THE RESPONSE

# EXT: THE FEB.MINT CI CONTAINS 0, SO THIS PREDICTOR HAS NO RELATIONSHIP
# WITH THE RESPONSE 
#
# Estimate confidence intervals for coefficients for detection submodel:
confint( fm.dyn, type = 'det' )
# How do we interpret these results?
# Answer: THE INT (OBSERVERTECH1) AND SAGEBRUSH CI's CONTAINS 0, SO THESE PREDICTORS HAVE NO RELATIONSHIP
# WITH THE RESPONSE
#
# How are they different to our single-season detection model?
# Answer: IN THE SINGLE SEASON MODEL, THE CIs FOR OBSERVER1, OBSERVER2, AND
# SAGEBRUSH CONTAIN 0. IN THE ROBUST MODEL, THE CIs FOR OBSERVER1 AND 
# SAGEBRUSH CONTAIN 0. 

# ROBUST
# > confint( fm.dyn, type = 'det' )
# 0.025      0.975
# p(Int)            -0.19092471  0.2297247 # contains 0
# p(observertech.2) -0.98526409 -0.4299756
# p(observertech.3) -1.75774585 -1.1515644
# p(observertech.4) -2.65589694 -1.9195659
# p(sagebrush)      -0.04281024  0.1717198 # contains 0

# SINGLE SEASON
# > # coefficients for detection submodel:
#   > confint( fm.closed, type = 'det' )
# 0.025       0.975
# p(Int)        -1.08368314  0.02256344 # contains 0
# p(obsvtech.2) -0.70097128  0.84688912 # contains 0
# p(obsvtech.3) -1.94970330 -0.07626419
# p(obsvtech.4) -2.10398903 -0.31472679
# p(sagebrush)  -0.07657449  0.49115122 # contains 0
...
# Answer: THE TOTAL CHI-SQUARE VALUE OF ~76 IS QUITE LARGE, INDICATING THAT 
# THERE IS A LARGE DIFFERENCE BETWEEN THE OBSERVED AND EXPECTED VALUES

# THE P-VALUE OF 0.525 IS ALSO VERY LARGE, INDICATING THAT THE RESULTS ARE NOT
...
# Answer: USING 4 AS A CUT-OFF (MORE OR LESS ARBITRARY), ONLY 3 SEASONS HAD A 
# CHI-SQUARE VALUE BELOW 4: SEASONS 3, 4, AND 9. THE REST HAD A CHI-SQUARE VALUE
# OF 4 OR GREATER, WITH THE MAXIMUM AT 10.6. MOST OF THE MODELS DID POORLY. 
#
...
# Answer: OCCUPATION IS DECLINING WITH TIME
# 
...
# AS SAGEBRUSH PERCENT COVER INCREASES, SO DOES COLONIZATION.
# Is there a potential threshold beyond which colonization becomes unlikely?
# Answer: YES. IT IS NOT LIKELY THAT COLONIZATION WOULD EVER REACH A TRUE VALUE 
# OF 100%. IT APPEARS THAT THE LINE BEGINS TO LEVEL OUT ABOVE 20% SAGEBRUSH 
# COVER. THIS IS CONSISTENT WITH RESEARCH WHICH SUGGESTS THAT THERE IS A POINT 
# OF DIMINISHING RETURNS ONCE SAGEBRUSH PERCENT COVER REACHES A CERTAIN LEVEL 
# (USUALLY AROUND 40%)
#
...
# EXTINCTIONG INCREASES AS CHEATGRASS PERCENT COVER INCREASE. THE EXTINCTION 
# RATE IS GRADUAL UNTIL ~10% COVER, THEN INCREASES CHARPLY BETWEEN ~10% - 25%, 
# THEN TAPERS OFF AROUND 25%. 
# Answer:
# 
...
# Answer: observer and sagebrush 
sageDet <- data.frame( observer = factor( "tech.1", levels = c("tech.1", "tech.2",
                                                          "tech.3", "tech.4") ), sagebrush = sage.std )
#predict partial relationship between sagebrush and detection
pred.det.sage <- predict( fm.dyn, type = "det", newdata = sageDet, 
                          appendData = TRUE )
#now for observer effects:
obsvDet <- data.frame( observer = factor( c("tech.1", "tech.2","tech.3", "tech.4"), 
                                      levels = c("tech.1", "tech.2","tech.3", "tech.4") ), 
                       sagebrush = 0 )
#predict partial relationship between observer effects and detection
pred.det.obsv <- predict( fm.dyn, type = "det", newdata = obsvDet, 
                          appendData = TRUE )

mysage <- cbind( pred.det.sage[,c("Predicted", "lower", "upper") ], sagebrush ) %>%
  # define x and y values
  ggplot(., aes( x = sagebrush, y = Predicted ) ) + 
  #choose preset look
  theme_bw( base_size = 15 ) +
  # add labels
  labs( x = "Sagebrush (%)", y = "Estimate Detection" ) +
  # add band of confidence intervals
  geom_smooth( aes(ymin = lower, ymax = upper ), 
               stat = "identity",
               size = 1.5, alpha = 0.5, color = "grey" ) +
