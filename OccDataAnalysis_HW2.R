...
# Answer: ONLY ONE YEAR OF DATA, SO THERE WILL BE NO VARIABILITY
#
...
# Answer: EASE OF INTERPRETATION AND ENSURE EQUAL CONTRIBUTION TO MODEL
#
...
# NUMBER OF SITES
# MAX AND MEAN OBS PER SITE
# SITES WITH AT LEAST ONE OBS
# SITE-LEVEL COVARIATES - DESCRIPTIVE STATS
# OBS LEVEL COVARIATES - NUMBER OF OBVS
...
# Answer: THE COEFFICIENTS DESCRIBE THE PREDICTORS EFFECT ON THE MODEL
#
...
( fm.7 <- occu( ~ 1 + obsv ~ 1 + cheatgrass, data = umf ) ) # model did not converge
...
( fm.12 <- occu( ~ 1 + sagebrush ~ 1, data = umf ) ) # model did not converge
( fm.13 <- occu( ~ 1 ~ 1 + sagebrush + cheatgrass, data = umf ) ) # model did not converge
( fm.14 <- occu( ~ 1 ~ 1 + sagebrush , data = umf ) )
( fm.15 <- occu( ~ 1 ~ 1 + cheatgrass, data = umf ) ) # model did not converge
...
# Answer: USING THE AIC VALUE, MODEL FM.3 WAS MOST SUPPORTED FROM THE MANUAL METHOD
# AND FM.8 WAS MOST SUPPPORTED FROM THE DREDGE METHOD
#
...
y.est.fm.8 <-round( bup(ranef(fm.8), stat="mean" ) ) # Posterior mean
...
#view together
data.frame( y.naive, y.est.fm.closed, y.est.fm.8, y.est.fm.16 )
