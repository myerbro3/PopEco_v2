...
# Answer: USING LEFT JOIN PRESERVE THE ROWS IN THE X POSITION, RIGHT 
# JOIN PRESERVES THE ROWS IN THE Y POSITION. I TESTED THIS TO SEE THE DIFFERENCE
# LEFT_JOIN KEEPS ALL ROWS AND COLUMNS WHILE RIGHT_JOIN ONLY KEEPS O.SITES, 
# COUNTED, YEAR, AND THE COUNT, OBSERVER, AND TIME COLUMNS, 
#
...
opendf %>% group_by( year, o.sites ) %>%
  ...
  # why?
  # ANSWER THE NAIVE ESTIMATE DOES NOT TAKE INTO ACCOUNT THE OBSERVER OR 
  # ENVIRONMENTAL PREDICTORS. IT IS STILL USEFUL FOR VIEWING OBSERVATIONS OVER 
  # TIME. ALL ELESE BEING EQUAL, A TREND IN THE OBSERVATIONS CAN BE INFORMATIVE 
  # AND GUIDE WHAT ADDITIONAL QUESTONS SHOULD BE ASKED OR WHAT VARIABLES MAY 
  # BE IMPACTING ABUNDANCE
  # 
...
# site occupancy? 
# Answers:
# IN GENERAL, IT APPEARS ABUNDANCE HAS A SLIGHT DOWNWARD TREND, THOUGH SOME 
# SITES HAVE A SIGNIFICANT INCREASE. DIFFICULT TO SEE WITH 50 SITES ON ONE GRAPH.
# IF I HAVE TIME, I WILL RETURN TO THIS AND PLOT FEWER SITES PER GRAPH. 
#
...
# We create our closeddf USING 2010:
closeddf <- opendf %>% 
...
write.csv( opendf, paste( getwd(),"/Week_8/Data/open_counts.csv", sep = "" ),  
           row.names = FALSE )

#save closed dataframe in our data folder:
write.csv( closeddf, paste( getwd(),"/Week_8/Data/closed_counts.csv", sep = "" ),  
           row.names = FALSE )
